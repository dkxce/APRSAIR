...
namespace ReadWave
{
    public class DirectAudioAFSKDemodulator
    {
        ...
        public bool OutLogToConsole = false;
        ...
        public DirectAudioAFSKDemodulator(int deviceNo, ax25kiss.PacketHandler h)
        {
            // WaveInDevices();
        ...
        public void Start()
        {
            if (OutLogToConsole)
            {
                Console.WriteLine("Start AFSK Listen {0}...", WaveInDevices()[recored.DeviceNumber]);
            };
        ...
        private void recored_DataAvailable(object sender, WaveInEventArgs e)
        {
            ...
            for (int i = 0; i < samples.Length; i++)
                samples[i] = (float)ReadWave.WaveStream.bytesToDouble(e.Buffer[pos++], e.Buffer[pos++]);
